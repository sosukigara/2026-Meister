...
def _pkg_root() -> str:
    """Return source tree root (fallback when ament_index isn't available)."""
    return os.path.normpath(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), '..'))
...
def _bt_path(name: str) -> str:
    ...
    candidates = [
        ...
        os.path.join(_pkg_root(), 'config', 'bt', f'{name}.xml'),
    ]
    for p in candidates:
        if os.path.isfile(p):
            return p
...
def _no_go_yaml_path() -> str:
    ...
    candidates = [
        ...
        os.path.join(_pkg_root(), 'config', 'no_go_zones.yaml'),
    ]
    for p in candidates:
        if os.path.isfile(p):
            return p
...
def _bt_dir() -> str:
    ...
    if os.path.isdir(p):
        return p
    return os.path.join(_pkg_root(), 'config', 'bt')
