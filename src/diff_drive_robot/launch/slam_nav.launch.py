...
def _build_runtime_actions(context, pkg_share: str):
    ...
    # Generate URDF string from xacro for robot spawn
    urdf_xacro_path = os.path.join(pkg_share, 'urdf', 'robot.urdf.xacro')
    try:
        robot_urdf_str = subprocess.check_output(
            ['xacro', urdf_xacro_path], timeout=15, text=True)
    ...
    # ── Perception Layer: YOLO object detection + human tracking ──────────────
    ...
    # ── Fusion Node: perception → velocity command integration ────────────────
