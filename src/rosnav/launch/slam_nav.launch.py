...
def _build_runtime_actions(context, pkg_share: str):
    ...
    urdf_override = LaunchConfiguration('urdf').perform(context).strip()
    rsp_urdf = urdf_override if urdf_override else os.path.join(pkg_share, 'urdf', 'robot.urdf.xacro')
    ...
    rviz_cfg = rviz_config_override if rviz_config_override else os.path.join(pkg_share, 'rviz', 'bot.rviz')
    rviz2 = GroupAction(
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            name='urdf', default_value='',
            description='Override URDF xacro path (empty = use default)'),
        DeclareLaunchArgument(
            name='params_file', default_value='',
            description='Override Nav2 params YAML path (empty = use default)'),
        DeclareLaunchArgument(
            name='rviz_config', default_value='',
            description='Override RViz config path (empty = use default)'),
        DeclareLaunchArgument(
            name='drive_type', default_value='diff_drive',
            description='Drive base type: diff_drive or mecanum'),
